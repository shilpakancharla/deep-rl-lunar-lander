...
from replay_buffer import ReplayBluffer

def train(sess, env, actor, critic, actor_noise, buffer_size, min_batch, ep):
    sess.run(tf.global_variables_initializer())

    # Initialize target network_weights
    actor.update_target_network()
    critic.update_target_network()

    # Initialize replay memory
    replay_buffer = ReplayBluffer(buffer_size, 0)
    ...
    for i in range(max_episodes):
        ...
        for j in range(max_steps):
            ...
            replay_buffer.add(np.reshape(state, (actor.state_dim,)), np.reshape(actor, (actor.action_dim,)), reward,
            done, np.reshape(next_state, (actor.state_dim,)))
